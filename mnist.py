...
slice_size  = 20000 #-1 #10000
n_hidden    = 2000 # Accuracy: 97.15% #1000 Accuracy: 94%

# 1. Load Data
mnist = datasets.MNIST(root=root, train=True, download=True)

X = mnist.data.numpy().reshape(-1, 28*28).astype(np.float32) / 255.0
Y = np.eye(10)[mnist.targets.numpy()] # One-hot labels
...
print(f'Galactic 2-Layer Shapes X:{X.shape} Y:{Y.shape} W1: {W1.shape} B1:{b1.shape} H:{n_hidden} Samples:{slice_size}')
...
# 4. Instant Inference
X_test = datasets.MNIST(root=root, train=False).data.numpy().reshape(-1, 28*28) / 255.0

print (f'Inference. Shapes X-test: {X_test.shape}')
 
t0       = time.time()
H_test   = np.maximum(0, X_test @ W1 + b1)
accuracy = np.mean(np.argmax(H_test @ W2_galactic, axis=1) == datasets.MNIST(root=root, train=False).targets.numpy())
e0       = time.time() - t0

print(f"Galactic 2-Layer Accuracy: {accuracy * 100:.2f}% elpased:{e0:.2f}s")
